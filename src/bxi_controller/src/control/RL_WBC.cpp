...
RobotController::RobotController() : Node("RobotController")
{
    ...
    load_config("src/bxi_controller/config/rl_wbc.yaml");
...
void RobotController::update_obs()
{
    ...
    {   obs_cur_.setZero();
        ...
        Eigen::VectorXf joint_pos = (joint_pos_.cast<float>()- default_dof_pos_)*obs_scales_["dof_pos"];
        Eigen::VectorXf joint_vel = joint_vel_.cast<float>() * obs_scales_["dof_vel"];
        
        Eigen::VectorXf reordered_joint_pos(joint_num_);  // 按照新顺序重组
        Eigen::VectorXf reordered_joint_vel(joint_num_);  // 按照新顺序重组
        reordered_joint_pos << joint_pos.segment(0, 6), joint_pos.segment(12, 4), 
                                joint_pos.segment(6, 6), joint_pos.segment(16, 4);
        reordered_joint_vel << joint_vel.segment(0, 6), joint_vel.segment(12, 4),
                                joint_vel.segment(6, 6), joint_vel.segment(16, 4);
        ...
        obs_cur_.segment(12, 20) = reordered_joint_pos;
        obs_cur_.segment(32, 20) = reordered_joint_vel;
        obs_cur_.segment(52, 20) =  action_output_;
...
void RobotController::infer_loop()
{
    ...
    while (rclcpp::ok() && run_)
    {
        ...
        Eigen::VectorXf reordered_action_output(action_dim_);  // 按照新顺序重组
        reordered_action_output << action_output_.segment(0, 6), action_output_.segment(10, 6),
                                    action_output_.segment(6, 4), action_output_.segment(16, 4);
        target_joint_pos += reordered_action_output * action_scales_ ;
